...
exports.MainPage = class MainPage {
  constructor(page) {
    this.page = page;
    this.donationWidgetFrameLocator = page.frameLocator(
    ...
    this.monthlyBtn = this.donationWidgetFrameLocator.getByRole("button", {
      name: "Monthly",
    });
    this.donateMonthlyBtn = this.donationWidgetFrameLocator.locator(
      '[data-qa="donate-button"]'
    );
    this.creditCardBtn = this.donationWidgetFrameLocator.getByRole("button", {
      name: "Credit card",
    });
    this.continueBtn = this.donationWidgetFrameLocator.getByRole("button", {
      name: "Continue",
    });
    ...
    //Inputs:
    this.priceInput =
      this.donationWidgetFrameLocator.getByTestId("price-input");
    this.cardNumberInput = this.donationWidgetFrameLocator
      .frameLocator('iframe[title="Secure card number input frame"]')
      .getByPlaceholder("Card number");
    this.monthYearInput = this.donationWidgetFrameLocator
      .frameLocator('iframe[title="Secure expiration date input frame"]')
      .getByPlaceholder("MM / YY");
    this.cvcInput = this.donationWidgetFrameLocator
      .frameLocator('iframe[title="Secure CVC input frame"]')
      .getByPlaceholder("CVC");
    this.firstNameInput = this.donationWidgetFrameLocator.getByTestId(
      "privacy-first-name-input"
    );
    this.lastNameInput = this.donationWidgetFrameLocator.getByTestId(
      "privacy-last-name-input"
    );
    this.emailInput = this.donationWidgetFrameLocator.getByTestId(
      "privacy-email-input"
    );

    //Lists:
    this.currencyList = this.donationWidgetFrameLocator.locator(
      '[data-qa="currency-selector"]'
    );

    //Check-boxes:
    this.coverTransactionCostCheckBox =
      this.donationWidgetFrameLocator.getByRole("checkbox", {
        name: "Cover transaction costs",
      });

    //Labels:
    this.cardContinueErrorMessageLabel =
      this.donationWidgetFrameLocator.locator(
  ...
  async typePriceInput(price) {
    ...
    await this.priceInput.press("Meta+A");
    await this.priceInput.press("Delete");
  ...
  async selectFromCurrencyList(currency) {
    await this.currencyList.waitFor();
  ...
  async cardContinueErrorMessageLabelHasText(errorText) {
    const actualErrorText =
      await this.cardContinueErrorMessageLabel.textContent();
